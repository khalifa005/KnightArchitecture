...
public class GenericRepository<T> : IGenericRepository<T> where T : BaseEntity
{
  ...
  public async Task<IReadOnlyList<T>> GetAllAsync(Func<IQueryable<T>, IIncludableQueryable<T, object>> include = null, CancellationToken cancellationToken = default)
  {
    var query = ApplyIncludes(include);
    return await query.ToListAsync(cancellationToken);
  }

  public async Task<List<T>> GetAllWithTrackingAsync(Func<IQueryable<T>, IIncludableQueryable<T, object>> include = null, CancellationToken cancellationToken = default)
  {
    var query = ApplyIncludes(include, tracking: true);
    return await query.ToListAsync(cancellationToken);
  }
  ...
  public async Task<PagedList<T>> GetPagedAsync(int pageNumber, int pageSize, Expression<Func<T, bool>> expression, Func<IQueryable<T>, IIncludableQueryable<T, object>> include = null, CancellationToken cancellationToken = default)
  {
    IQueryable<T> query = _dbContext.Set<T>();
