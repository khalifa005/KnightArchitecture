...
import { atLeastOne, fileExtensionsValidator, maxFilesValidator, requiredFileUploadValidationSizeInMB } from '../../../../@core/utils.ts/form/validations/form.validation-helpers';
...
import { WorkingDayForm } from './components/working-days-form/working-day-form';

export class AdvancedExampleForm extends FormGroup {
  ...
  // New FormArray for working days
  readonly workingDaysArray = this.get('workingDays') as FormArray;
  ...
  constructor(fb: FormBuilder = new FormBuilder(),
    ...
    super(
      fb.group({
        inputControl: [model.inputControl || '', [
          ...
          AdvancedExampleForm.regexValidator(/^[a-zA-Z0-9]*$/)
        ]],
        ...
        dateControl: [model.dateControl || '', [
          Validators.required,
          AdvancedExampleForm.dateRangeValidator(new Date('2023-01-01'), new Date('2024-12-31')),
        ]],
        ...
        dropdownControl: [model.dropdownControl || null, [Validators.required, AdvancedExampleForm.valueGreaterThanZeroValidator]],
        multiSelectControl: [model.multiSelectControl || [], Validators.required],
        hoursControl: [model.hoursControl || null, [Validators.required, AdvancedExampleForm.valueGreaterThanZeroValidator]],
        ...
        // workingDays: fb.array([]), // Initialize the FormArray
        workingDays: fb.array([], [atLeastOne]), // Add the custom validator here
    ...
    this.initializeWorkingDays(model.workingDays || []);
  ...
  private initializeWorkingDays(workingDays: any[]): void {
    workingDays.forEach(day => {
      this.workingDaysArray.push(new WorkingDayForm(new FormBuilder(), day));
    });
  }
  
    // Method to add a new working day entry
    addWorkingDay(): void {
      const fb = new FormBuilder();
      this.workingDaysArray.push(
        fb.group({
          workingDate: [null, [Validators.required, AdvancedExampleForm.dateRangeValidator(new Date('2023-01-01'), new Date('2024-12-31'))]],
          fromWorkingHour: [null, [Validators.required, Validators.min(0), Validators.max(23)]],
          toWorkingHour: [null, [Validators.required, Validators.min(0), Validators.max(59)]],
        })
      );
    }
    // Method to remove a working day entry by index
    removeWorkingDay(index: number): void {
  ...
  private updateMinutesValidation(hoursValue: number): void {
    if (hoursValue === AppDefaultValues.DropDownAllOption || !hoursValue) {
      ...
      this.minutesControl.setValidators([Validators.required, AdvancedExampleForm.valueGreaterThanZeroValidator]);
    }
