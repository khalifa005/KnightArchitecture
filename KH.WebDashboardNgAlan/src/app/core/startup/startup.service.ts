...
export class StartupService {
  ...
  load(isStaticLogin: boolean = false, appData: string='app-data.json'): Observable<void> {
    ...
    return zip(this.i18n.loadLangData(defaultLang), this.httpClient.get(`./assets/tmp/${appData}`)).pipe(
      ...
      map(([langData, appData]: [Record<string, string>, NzSafeAny]) => {
        ...
        if (!isStaticLogin && storedUser?.permissions) {
          // Apply permissions from storage
          this.aclService.setAbility(storedUser.permissions);
          this.filterMenuByPermissions();
        } else if (isStaticLogin && storedUser?.permissions) {
          // For static login, use the permissions that were already set
          this.aclService.setAbility(storedUser.permissions);
          this.filterMenuByPermissions();
        } else {
          // Use static user for initial load (no user logged in yet)
